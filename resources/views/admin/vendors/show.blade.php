...
<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-900/5">
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-900/5">
            <div class="text-center">
                <img src="{{ $vendor->vendorDetail->profile_photo }}"
                    class="mx-auto h-32 w-32 rounded-full border-4 border-slate-200 object-cover shadow-sm shadow-slate-900/10">
                <h3 class="mt-4 text-xl font-semibold text-slate-900">{{ $vendor->vendorDetail->shop_name ?? 'N/A' }}</h3>
                <span class="mt-2 inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
                    Vendor ID: #{{ $vendor->id }}
                </span>
            </div>
        ...
        <div class="xl:col-span-2 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-900/5" x-data="{ count: 1 }">
            <div class="mb-5 flex items-center gap-3 border-b border-slate-200 pb-3">
                <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-100 text-[#f5185a]">
            ...
            <form action="{{ route('admin.food.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <select name="category_id"
                            class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-800 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200">
                    ...
                    <div id="variants-container" class="mt-2 space-y-3">
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                            <input type="text" name="variants[0][name]" placeholder="Variant (Half/Full)"
                                class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-800 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200"
                                required>
                ...
                <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <button type="button" onclick="addVariant()"
                        class="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-[#f5185a] transition hover:bg-slate-50">
    ...
    <div class="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm shadow-slate-900/5">
        <div class="border-b border-slate-200 bg-white px-4 py-4">
            <h3 class="font-semibold text-slate-900">Vendor's Menu List</h3>
        </div>

        <table class="w-full text-left text-sm">
            <thead class="bg-white text-xs uppercase tracking-wider text-slate-900">
                <tr>
                    <th class="border-b border-slate-200 p-4"><i class="fa-solid fa-bowl-food mr-2"></i>Food Name</th>
                    <th class="border-b border-slate-200 p-4"><i class="fa-solid fa-layer-group mr-2"></i>Category</th>
                    <th class="border-b border-slate-200 p-4"><i class="fa-solid fa-tags mr-2"></i>Variants (Price)</th>
                    <th class="border-b border-slate-200 p-4 text-center"><i class="fa-solid fa-gear mr-2"></i>Action</th>
                </tr>
            </thead>
